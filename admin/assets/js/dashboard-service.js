document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchTransactions() {
        try {
            ...
            return snapshot.docs.map(doc => {
                ...
                const paymentStatus =
                    remainingBalance > 0
                        ? `Bal: ₱${remainingBalance.toLocaleString()}`
                        : "Fully Paid";
    ...
    function calculateRentalStatus(transaction) {
        ...
        const eventStartDate = transaction.eventStartDate
            ? new Date(transaction.eventStartDate)
            : null;
        const eventEndDate = transaction.eventEndDate
            ? new Date(transaction.eventEndDate)
            : null;
        const eventDate = transaction.eventDate
        ...
        if (rentalEndDate) {
            ...
            if (today < rentalStartDate) {
                ...
                const daysDiff = Math.floor(
                    (today - rentalStartDate) / (1000 * 60 * 60 * 24)
                );
    ...
    async function showRentalDetails(id) {
        try {
            ...
            const transactionDate = data.timestamp
            ...
            document.getElementById("customer-name").textContent =
                data.fullName || data.customerName || "Not provided";
            ...
            document.getElementById("customer-address").textContent =
                data.address || data.customerAddress || "Not provided";

            // Populate transaction details
            document.getElementById("transaction-code").textContent =
                data.transactionCode || "Not assigned";

            // Handle rental type and dates - check multiple possible field names
            const rentalType =
                data.rentalType || data.status || data.type || "Unknown";
            console.log(
                "Rental Type from Firebase:",
                rentalType,
                "Full data:",
                data
            );
            ...
            rentalTypeElement.textContent =
                getRentalTypeDisplayName(rentalType);
            rentalTypeElement.className = `status-badge ${getRentalTypeClass(
                rentalType
            )}`;
            ...
            const rentalFee = parseFloat(
                data.rentalFee || data.totalPayment || 0
            );
            const totalPayment = parseFloat(data.totalPayment || 0);
            const remainingBalance = parseFloat(data.remainingBalance || 0);
            ...
            document.getElementById(
                "rental-fee"
            ).textContent = `₱${rentalFee.toLocaleString()}`;
            document.getElementById(
                "payment-total"
            ).textContent = `₱${totalPayment.toLocaleString()}`;
            document.getElementById(
                "payment-remaining"
            ).textContent = `₱${remainingBalance.toLocaleString()}`;
            ...
            notesElement.textContent =
                ...
                "No additional notes provided."; // Show modal
            modal.classList.add("show");
            modal.style.display = "flex";
        } catch (error) {
            hideModalLoadingState();
    ...
    function setupRentalDates(data, rentalType) {
        const rentalDatesSection = document.getElementById(
    ...
    async function populateRentedItems(data) {
        ...
        itemsContainer.innerHTML =
            '<div class="loading-items"><i class="fas fa-spinner fa-spin"></i> Loading items...</div>';
        ...
        try {
                    ...
                    data.accessories.map(async accessory => {
                        try {
                            ...
                            if (
                                ...
                                accessoryData.imageUrl.trim() !== ""
                            ) {
                                imageUrl = accessoryData.imageUrl;
                            } else if (
                                accessoryData.frontImageUrl &&
                                accessoryData.frontImageUrl.trim() !== ""
                            ) {
                            ...
                            return {
                                ...
                                image: imageUrl,
                                price:
                                    accessory.price || accessoryData.price || 0,
                            ...
                            console.error(
                                "Error fetching accessory details:",
                                error,
                                "Accessory ID:",
                                accessory.id
                            );
            ...
            if (itemsHtml) {
                ...
                itemsContainer.innerHTML =
                    '<div class="empty-state"><i class="fas fa-box-open"></i><p>No items found for this rental.</p></div>';
            }
        } catch (error) {
            console.error("Error loading rental items:", error);
            itemsContainer.innerHTML =
                '<div class="error-state"><i class="fas fa-exclamation-triangle"></i><p>Error loading rental items.</p></div>';
        }
    } // Create HTML for individual rental item
    function createItemHtml(item) {
        const inclusionsHtml =
            item.inclusions && item.inclusions.length > 0
                ? `<span><strong>Inclusions:</strong> ${item.inclusions.join(
                      ", "
                  )}</span>`
                : "";
            ...
            <div class="rental-item">
                ...
                <div class="item-details">
                    ...
                    <div class="item-info">
                        ...
                        ${
                            item.sizes
                                ? `<span><strong>Sizes:</strong> ${item.sizes}</span>`
                                : ""
                        }
                        ${
                            item.category
    ...
    async function updateDashboardCards() {
        try {
            ...
            document.getElementById("total-rentals-count").innerHTML =
                '<i class="bx bx-loader-alt bx-spin"></i>';
            document.getElementById("total-appointments-count").innerHTML =
                '<i class="bx bx-loader-alt bx-spin"></i>';
            document.getElementById("total-products-count").innerHTML =
                '<i class="bx bx-loader-alt bx-spin"></i>'; // Ensure rentals data is loaded first
            ...
            document.getElementById("total-rentals-count").textContent =
                totalRentalsCount; // Update Total Appointments count (all appointments in the system)
            const allAppointments = await fetchAllAppointments();
            const totalAppointmentsCount = allAppointments.length;
            document.getElementById("total-appointments-count").textContent =
                totalAppointmentsCount; // Update Total Products count (all products in inventory)
        ...
        try {
            ...
            appointmentsUnsubscribe = db
                ...
                .onSnapshot(
                    snapshot => {
                        console.log(
                            "📡 Real-time appointments update received"
                        );
                    ...
                    error => {
                        console.error(
                            "Error in appointments real-time listener:",
                            error
                        );
                    }
                );
            ...
            console.log(
                "✅ Appointments real-time listener set up successfully"
            );
        } catch (error) {
            console.error(
                "Error setting up appointments real-time listener:",
                error
            );
        }
    }
    ...
    function renderAppointmentsFromData(appointments) {
        ...
        document
            .querySelectorAll(".appointment-view-details")
            .forEach(button => {
    ...
    async function showAppointmentDetails(id) {
        try {
            ...
            const confirmedStatusTag = document.getElementById(
                "appointment-confirmed-tag"
            );
            const cancelledStatusTag = document.getElementById(
                "appointment-cancelled-tag"
            );
                ...
                products = cartItems
                    ...
                    <tr>
                        <td><img src="${item.image}" alt="${
                            item.name
                        }" class="appointment-product-img"></td>
                        <td>
                            <div class="appointment-product-info">
                                <div class="appointment-product-name">${
                                    item.name
                                }</div>
                                <div class="appointment-product-details">Size: ${
                                    item.size
                                } | Qty: ${item.quantity}</div>
                            </div>
                        </td>
                        <td class="appointment-product-price">₱ ${(
                    ...
                    <div class="appointment-customer-info">
                        ...
                        <div class="appointment-info-item">
                            ...
                            <span class="appointment-info-value">${
                                customerRequest || "No special requests"
                            }</span>
                        </div>
                    </div>
    ...
    document.addEventListener("click", function (e) {
        ...
        if (
            e.target.classList.contains("appointment-view-details") ||
        ...
        if (
            e.target.classList.contains("close-rental-modal") ||
        ...
        if (e.target.classList.contains("appointment-view-details")) {
            e.preventDefault();
            console.log(
                "🔍 Appointment view details button clicked!",
                e.target
            );
            ...
            if (appointmentId) {
                console.log(
                    "🔍 Opening appointment details for ID:",
                    appointmentId
                );
    ...
    $(document).on("click", ".cancel-booking", function () {
        ...
        console.log(
            "Cancel booking clicked, appointmentId:",
            appointmentId,
            "status:",
            status
        );
    ...
    $(document).on("click", ".confirm-booking", function () {
        ...
        console.log(
            "Confirm booking clicked, appointmentId:",
            appointmentId,
            "status:",
            status
        );
    ...
    $(document).on(
        ...
        function () {
            const appointmentId = $("#confirm-booking-modal").data(
                "appointmentId"
            );
...
$(document).on("click", ".modal-backdrop", function (e) {
    if ($(e.target).hasClass("modal-backdrop")) {
        const clickedInConfirmationModal =
            $(this).closest(
                "#cancel-booking-modal, #confirm-booking-modal, #undo-confirmation-modal, #undo-cancellation-modal"
            ).length > 0;
...
function showSampleAppointmentDetails(button) {
    try {
        ...
        const appointmentText =
            appointmentItem.querySelector(".appointment-text").textContent;
        ...
        console.log(
            "🔍 Sample appointment status detected:",
            appointmentStatus
        );
        ...
        const sampleAppointmentId = `sample-${customerName
            .replace(/\s+/g, "-")
            .toLowerCase()}-${appointmentDate
            .replace(/\s+/g, "-")
            .toLowerCase()}`;
        ...
        document.getElementById("rental-fee").textContent = `₱${(
            Math.random() * 1000
        ).toFixed(2)}`;
        ...
        setTimeout(() => {
            ...
            // Update buttons based on appointment status
            console.log(
                "🔄 Calling updateAppointmentModalButtons with status:",
                appointmentStatus
            );
            updateAppointmentModalButtons(modal, appointmentStatus);
...
$(document).ready(function () {
    ...
    $(
        "#confirm-booking-modal, #cancel-booking-modal, #undo-confirmation-modal, #undo-cancellation-modal"
    ).each(function () {
        const modal = $(this)[0];
        modal.style.display = "none";
        modal.style.visibility = "hidden";
