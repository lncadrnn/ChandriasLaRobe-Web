...
$(document).ready(function () {
    ...
    */
    ...
    function createProductHTML(product) {
        ...
        <div class="product-item">
            ...
            <div class="product-banner">                <a href="details.html?id=${product.id}" class="product-images">
                    ...
                    <button class="action-btn view-availability-btn" aria-label="View Availability" data-product-id="${product.id}">
        ...
        $("body").append(`
            <div id="availability-modal" class="modal-overlay">
                <div class="modal-container availability-modal">
                    <button id="close-availability-modal" class="close-btn" aria-label="Close">
                    ...
                    <div class="modal-header">
                        ...
                        <h3 class="modal-title">Product Rental Schedule</h3>
                    </div>
                    <div class="modal-body">
                        <div id="availability-list">
                            <div class="loading-spinner">
                                <div class="spinner"></div>
                                <span>Loading rental dates...</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    // Show tooltip on hover (optional, for custom tooltip)
    $(document).on('mouseenter', '.view-availability-btn', function() {
        $(this).attr('title', 'View availability');
    });

    // Show modal and fetch dates on click
    $(document).on('click', '.view-availability-btn', async function() {
        const productId = $(this).data('product-id');
        showAvailabilityModal();
        await fetchAndDisplayAvailability(productId);
    });
    ...
    $(document).on('click', '#close-availability-modal', hideAvailabilityModal);
    $(document).on('click', '#availability-modal', function(e) {
        if (e.target === this) hideAvailabilityModal();
    });
    $(document).on('keydown', function(e) {
        if (e.key === 'Escape' && $('#availability-modal').is(':visible')) hideAvailabilityModal();
    });

    // Fetch and display rented dates from Firestore
    async function fetchAndDisplayAvailability(productId) {
        ...
        try {
            ...
            // Fetch ONLY from appointments collection (admin calendar)
            ...
            appointmentsSnapshot.forEach(docSnap => {
                ...
                if (data.status && (data.status.toLowerCase() === 'cancelled' || data.status.toLowerCase() === 'completed')) {
                    console.log('Skipping appointment with status:', data.status);
                    return;
                }
                ...
                if (appointmentDate) {
                    ...
                    if (apptDate < currentMonth) {
                        console.log('Skipping old appointment from:', apptDate.toLocaleDateString());
                        return;
                    }
                }
                ...
                if (Array.isArray(data.cartItems)) {
                    ...
                    if (hasProduct) {
                        console.log('Found product in appointment:', data);
            ...
            try {
                console.log('Attempting to fetch from transaction/admin calendar collections...');
                ...
                for (const collectionName of possibleCollections) {
                    try {
                        ...
                        console.log(`Collection '${collectionName}' found with ${transactionSnapshot.size} documents`);
                        
                        if (transactionSnapshot.size > 0) {
                            // Show sample data for debugging
                            const sampleDoc = transactionSnapshot.docs[0];
                            console.log(`Sample data from '${collectionName}':`, sampleDoc.data());
                        }
                        ...
                        transactionSnapshot.forEach(docSnap => {
                            ...
                            if (productFound) {
                                console.log(`Found product in ${collectionName}:`, data);
                        ...
                        // If we found data in this collection, we can break or continue to check others
                        if (transactionSnapshot.size > 0) {
                            console.log(`Successfully processed ${collectionName} collection`);
                        }
            ...
            if (rentedDates.length === 0) {
                $list.html(`
                    <div class="no-rental-dates">
                        <i class="fi fi-rs-check-circle"></i>
                        <h4>Available</h4>
                        <p>This product is currently available for all dates.</p>
                    </div>
                `);
            } else {
                const datesList = rentedDates.map(date => `
                ...
                $list.html(`
                    <div class="rental-dates-container">
                        <h4>Unavailable Dates (${rentedDates.length})</h4>
                        <div class="rental-dates-list">
                            ${datesList}
                        </div>
                        <p class="rental-note">
                            <i class="fi fi-rs-info"></i>
                            These dates show when the product is currently rented or unavailable.
                        </p>
                    </div>
                `);
            }
            ...
            $list.html(`
                <div class="error-message">
                    <i class="fi fi-rs-exclamation"></i>
                    <h4>Error Loading</h4>
                    <p>Failed to fetch rental dates. Please try again.</p>
                </div>
            `);
        }
    }
    
    // Add tooltips for product action buttons (desktop only)
    $(document).on('mouseenter', '.action-btn', function() {
        // Only show tooltips on desktop devices
        if (window.innerWidth <= 768) return;
        
        const $btn = $(this);
        let tooltipText = '';
        
        if ($btn.hasClass('quick-view-btn-trigger')) {
            tooltipText = 'Quick View';
        } else if ($btn.hasClass('add-to-favorites-btn')) {
            tooltipText = 'Add to Favorites';
        } else if ($btn.hasClass('view-availability-btn')) {
            tooltipText = 'View Availability';
        }
        ...
        if (tooltipText) {
            ...
            tooltip.css({
                position: 'absolute',
                top: btnOffset.top - tooltip.outerHeight() - 8,
